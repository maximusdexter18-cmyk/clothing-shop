          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex gap-2 overflow-x-auto hide-scrollbar flex-1">
                <button onClick={() => setSelectedCategory(null)} className={`filter-pill whitespace-nowrap bg-white text-luxury-brown ${!selectedCategory ? "active" : ""}`}>All</button>
                {categories.map((cat) => (
                  <button key={cat} onClick={() => setSelectedCategory(selectedCategory === cat ? null : cat)} className={`filter-pill whitespace-nowrap bg-white text-luxury-brown ${selectedCategory === cat ? "active" : ""}`}>{cat}</button>
                ))}
              </div>
